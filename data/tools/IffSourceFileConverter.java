...
class IffSourceFileConverter implements SourceFileConverter {
    ...
    public void convertToRawData(FileOutputStream data) throws IOException {
        ...
        data.write(raw);
        if (!this.sourceFile.getParamAsBoolean(ParamType.WITHOUT_MASK)) {
            data.write(mask);
        }
    }
    ...
    public void addToIndex(List<SimpleEntry<String, Long>> index, List<SimpleEntry<String, String>> constants)
        ...
        index.add(new SimpleEntry<>(label, Long.valueOf(raw.length)));
        if (!this.sourceFile.getParamAsBoolean(ParamType.WITHOUT_MASK)) {
            label = generateLabel(sourceFile.filename, "mask");
            index.add(new SimpleEntry<>(label, Long.valueOf(mask.length)));
        }
    }
    ...
    private static class BodyProcessor implements ChunkProcessor {
        ...
        public void process(FileInputStream src, IffSourceFileConverter uow) throws IOException {
            ...
            //
            if (!uow.sourceFile.getParamAsBoolean(ParamType.WITHOUT_MASK)) {
                createMask(uow);
            }
        }
